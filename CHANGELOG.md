...
- Fix `annotate_genome_snvs`/`annotate_mt_snvs` silently falling back to the non-CADD-annotated VCF if a CADD result is ever missing when CADD was requested, instead of failing the run [issue #868](https://github.com/nf-core/raredisease/issues/868)
