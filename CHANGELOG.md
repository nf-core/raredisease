...
- Replace `--call_interval` (Sentieon-only) with `--snv_call_region`, which restricts SNV calling to a given region for both DeepVariant and Sentieon and falls back to the padded `--target_bed` as processed by the pipeline (not the raw file) when not set [issue #740](https://github.com/nf-core/raredisease/issues/740) [PR #1028](https://github.com/nf-core/raredisease/pull/1028)
