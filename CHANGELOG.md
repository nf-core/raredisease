...
- Add a real (non-stub) test to `annotate_genome_snvs` using the new minimal 9-region GIAB dataset in `--gtf` VEP mode, and migrate `annotate_mt_snvs`'s existing real test from the old large-genome fixtures to the same minimal dataset; also fixes both subworkflows' local test configs to make VEP's `--cache`/`--offline`/`--merged`/`--dir_plugins` args conditional on whether a gtf input was supplied, matching `conf/modules/annotate_{genome_snvs,mt_snvs}.config` [issue #795](https://github.com/nf-core/raredisease/issues/795) [PR #984](https://github.com/nf-core/raredisease/pull/984)
