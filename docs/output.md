...
For male samples the sex chromosomes are called as haploid (`--haploid_contigs`). The sex used is the samplesheet `sex`, or the ngs-bits `SampleGender` estimate when `--sex_source` is `auto`/`estimated` (see [Estimated sex](usage.md#estimated-sex)).
...
Read counts are denoised against the female or male panel of normals (`gens_pon_female`/`gens_pon_male`) according to the sample's sex — the samplesheet `sex`, or the ngs-bits `SampleGender` estimate when `--sex_source` is `auto`/`estimated` (see [Estimated sex](usage.md#estimated-sex)).
