...
include { BCFTOOLS_ANNOTATE as BCFTOOLS_APPEND_MIVMIR_GICAM     } from '../../../modules/nf-core/bcftools/annotate/main'
...
workflow RANK_VARIANTS {
    ...
    if (rank_with_mivmir_gicam) {
        ...
        BCFTOOLS_APPEND_MIVMIR_GICAM(ch_merge_genmod_gicam)

        BCFTOOLS_APPEND_MIVMIR_GICAM.out.vcf
            .join(BCFTOOLS_APPEND_MIVMIR_GICAM.out.tbi, failOnMismatch: true)
            .set { ch_vcf_annotated_with_mivmir_gicam_gz_tbi }

        ch_vcf_annotated_with_mivmir_gicam_gz_tbi.map { meta, gz, _tbi ->
            return [meta, gz]
        }.set { ch_vcf }

        ch_vcf_annotated_with_mivmir_gicam_gz_tbi
            .map { meta, gz, tbi -> ['rank_and_filter/', [meta, gz, tbi]] }
            .set { ch_publish }
    }
