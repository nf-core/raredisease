...
workflow RANK_VARIANTS {
    ...
    main:
        ...
        if (process_with_sort) {
            ...
            ch_tbi = BCFTOOLS_SORT.out.tbi
        } else {
            ch_vcf = TABIX_BGZIPTABIX(GENMOD_COMPOUND.out.vcf).gz_index.map { meta, vcf, _tbi -> [meta, vcf] } //run only for SNVs
            ch_tbi = TABIX_BGZIPTABIX.out.gz_index.map { meta, _gz, tbi -> [meta, tbi] }
        }
